...
public import GranvilleMoore.Attr
public import GranvilleMoore.CollapsedCoeff
public import GranvilleMoore.ExplicitForm
public import GranvilleMoore.UnitQuotient
...
@[granville_moore "lem_master_expansion"]
...
@[granville_moore "lem_master_term_integral"]
...
@[granville_moore "thm_fermat_integral"]
...
@[granville_moore "thm_fermat_congruence"]
...
private theorem prod_pow_sub_pow_rat_eq_zero {p n j : ℕ} (h : n < j) :
    ∏ r ∈ range j, ((p : ℚ) ^ n - (p : ℚ) ^ r) = 0 :=
...
`C(j,2)`. -/
private theorem prod_pow_sub_pow_rat_eq_pow_mul {p n j : ℕ} (h : j ≤ n) :
...
@[granville_moore "thm_fermat_congruence_exceptional"]
